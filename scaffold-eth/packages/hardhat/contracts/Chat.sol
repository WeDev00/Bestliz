...
contract ChatStaking {
	...
	function getAmountOfTokensStakedByAddress(
		address user
	) public view returns (StakedTokens memory) {
		return staked[user];
	}
	...
	function unstake() external {
		require(
			block.timestamp >= startTime[msg.sender] + stakingDuration,
			"Errore: periodo di staking non ancora terminato"
		);
		require(
			haveStaked[msg.sender],
			"Error:sender never staked some tokens"
		);
		// Unstake your tokens
	...
	function slashUser(address userToSlash) public onlyOwner {
		IERC20(staked[userToSlash].tokenAddress).transfer(
			owner,
			staked[userToSlash].amount
		);
